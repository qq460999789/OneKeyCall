...
{   //获得文件列表
	...
	for(int i = 0; i<[delegate numberOfFiles]; ++i)
	{
        ...
        NSString* filesize = [delegate fileSizeAtIndex:i];
        NSString* filecreatedate = [delegate fileCreateTimeAtIndex:i];
        NSString* filedisplayname = [delegate fileDisplayNameAtIndex:i];
        NSString* file = [filename stringByReplacingOccurrencesOfString:@"'" withString:@"\\'"] ;//把`处理保留
		NSString* fileSize = [filesize stringByReplacingOccurrencesOfString:@"'" withString:@"\\'"] ;//把`处理保留
        NSString* fileCreateDate = [filecreatedate stringByReplacingOccurrencesOfString:@"'" withString:@"\\'"] ;//把`处理保留
        NSString* fileDisplayName = [filedisplayname stringByReplacingOccurrencesOfString:@"'" withString:@"\\'"] ;//把`处理保留
		[output appendFormat:@"{'name':'%@','size':'%@','Date':'%@','DisplayName':'%@', 'id':%d},", file,fileSize,fileCreateDate,fileDisplayName,i];
	}
	if ([output length] > 1)
	{
