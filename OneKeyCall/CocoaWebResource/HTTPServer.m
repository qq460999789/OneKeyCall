...
- (id)init
{
	if(self = [super init])
	{
		...
		// Configure default values for   service
...
- (NSString*)hostName
{
	struct ifaddrs *addrs;                  //结构体
	const struct ifaddrs *cursor;
	int error;
	error = getifaddrs(&addrs);             //得到地址结构体，返回error信息
	NSString *hostname = nil;
	
	if (error)
	{
//		NSLog(@"%@", gai_strerror(error));
        NSLog(@"%s", gai_strerror(error));  //打印错误信息
...
- (BOOL)stop
{
    ...
    NSLog(@"HTTP server on port %hu stop", port);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark Bonjour Delegate Methods(NSNetServiceDelegate):
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{
	NSString* docroot =[NSString stringWithFormat:@"%@/Documents/docroot", NSHomeDirectory()];
...
//    [manager createSymbolicLinkAtPath:docroot withDestinationPath:path error:&error];
    ...
    //----自修改--start
    BOOL bo = [manager createDirectoryAtPath:docroot withIntermediateDirectories:YES attributes:nil error:nil];
    NSLog(@"create dir--%d---path-%@",bo,docroot);
...
-(void)copyIndexHtml
{
    ...
    NSString *documentLibraryFolderPath = [documentsDirectory stringByAppendingPathComponent:@"/docroot/index.html"];
    if ([[NSFileManager defaultManager] fileExistsAtPath:documentLibraryFolderPath]) {
        ...
        NSString *resourceSampleImagesFolderPath =[[NSBundle mainBundle]
                                                   pathForResource:@"/docroot/index.html"
                                                   ofType:@"html"];
